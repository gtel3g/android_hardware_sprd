...
int adev_get_audiomodenum4eng(void)
{
    ...
    ALOGW("adev_get_audiomodenum4eng s_audionum:0x%0x\n",s_audiomode->num);
        ...
        const XML_Char **attr)
{
    ...
    if(strcmp(tag_name, "audiomode") == 0)
    {
        ...
        if (strcmp(attr[0], "num") == 0) {
            ALOGD("Thse num of audio mode  is '%s'", attr[1]);
    ...
    else if(strcmp(tag_name, "item") == 0)
    {
        if (state->audio_mode_item_info) {
            ...
            ALOGD("the mode name is '%s',index is '%s'", attr[1],attr[3]);
...
void stringfile2nvstruct(char *filename, void *para_ptr, int lenbytes)
{
    ...
    fclose(fpRead);
    {
        ...
        {
            ALOGW("%d ------> %s",i,aud_params_ptr[i].audio_nv_arm_mode_info.ucModeName);
        }
    }
    ALOGW("stringfile2nvstruct end\n");
    return;
}
